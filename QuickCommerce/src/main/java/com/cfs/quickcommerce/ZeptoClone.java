...
class DbInventoryStore implements InventoryStore {
    ...
    public void addProduct(Product prod, int qty) {
        int sku = prod.getSku();
        if (!products.containsKey(sku)) products.put(sku, prod);
        stock.put(sku, stock.getOrDefault(sku, 0) + qty);
    }

    @Override
    public void removeProduct(int sku, int qty) {
        if (!stock.containsKey(sku)) return;
        int currentQuantity = stock.get(sku);
        int remainingQuantity = currentQuantity - qty;
        if (remainingQuantity > 0) stock.put(sku, remainingQuantity);
        else { stock.remove(sku); products.remove(sku); }
    }
    ...
    public List<Product> listAvailableProducts() {
        ...
        for (Map.Entry<Integer,Integer> it : stock.entrySet()) {
            int sku = it.getKey();
            int qty = it.getValue();
            if (qty > 0 && products.containsKey(sku)) available.add(products.get(sku));
        }
        return available;
    }
}
...
class InventoryManager {
    ...
    public void removeStock(int sku, int qty) { store.removeProduct(sku, qty); }
    public int checkStock(int sku) { return store.checkStock(sku); }
    public List<Product> getAvailableProducts() { return store.listAvailableProducts(); }
}
...
class ThresholdReplenishStrategy implements ReplenishStrategy {
    ...
    public ThresholdReplenishStrategy(int threshold) { this.threshold = threshold; }
...
class DarkStore {
    ...
    public void runReplenishment(Map<Integer,Integer> itemsToReplenish) {
        if (replenishStrategy != null) replenishStrategy.replenish(inventoryManager, itemsToReplenish);
    }
    ...
    public void removeStock(int sku, int qty) { inventoryManager.removeStock(sku, qty); }
    public void addStock(int sku, int qty) { inventoryManager.addStock(sku, qty); }
    public void setReplenishStrategy(ReplenishStrategy strategy) { replenishStrategy = strategy; }
    public String getName() { return this.name; }
}
...
class DarkStoreManager {
    ...
    private DarkStoreManager() { darkStores = new ArrayList<>(); }

    public static DarkStoreManager getInstance() {
        if (instance == null) instance = new DarkStoreManager();
        return instance;
    }
    ...
    public List<DarkStore> getNearbyDarkStores(double ux, double uy, double maxDistance) {
        ...
        for (DarkStore ds : darkStores) {
            ...
            if (d <= maxDistance) distList.add(new Pair<>(d, ds));
        }
        distList.sort(Comparator.comparing(Pair::getKey));
        List<DarkStore> result = new ArrayList<>();
        for (Pair<Double,DarkStore> p : distList) result.add(p.getValue());
        return result;
    }
}
...
class Cart {
    ...
    public double getTotal() {
        double sum = 0;
        for (Pair<Product,Integer> it : items) sum += it.getKey().getPrice() * it.getValue();
        return sum;
    }

    public List<Pair<Product,Integer>> getItems() { return items; }
}
...
class DeliveryPartner {
    public String name;
    public DeliveryPartner(String n) { name = n; }
}
...
class OrderManager {
    ...
    public void placeOrder(User user, Cart cart) {
        ...
        for (Pair<Product,Integer> item : requestedItems) {
            ...
            if (firstStore.checkStock(sku) < qty) { allInFirst = false; break; }
        }
        ...
        if (allInFirst) {
            ...
            for (DarkStore store : nearbyDarkStores) {
                ...
                for (Map.Entry<Integer,Integer> entry : allItems.entrySet()) {
                    ...
                    if (qtyNeeded > takenQty) allItems.put(sku, qtyNeeded - takenQty);
                    else toErase.add(sku);
                }
        ...
        for (Pair<Product,Integer> item : order.items)
            System.out.println("    SKU " + item.getKey().getSku() + " (" + item.getKey().getName() + ") x" + item.getValue() + " @ ₹" + item.getKey().getPrice());
...
class ZeptoHelper {
    public static void showAllItems(User user) {
        ...
        for (Map.Entry<Integer,Double> entry : skuToPrice.entrySet()) {
            System.out.println("  SKU " + entry.getKey() + " - " + skuToName.get(entry.getKey()) + " @ ₹" + entry.getValue());
        }
    }
